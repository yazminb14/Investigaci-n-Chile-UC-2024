...
def save_to_sqlite(can_id, sensor_name, timestamp, value):
    conn = sqlite3.connect('vehicle_data.db')
    cursor = conn.cursor()
    cursor.execute('''
        ...
        while True:
            ...
            if message and len(message.data) == 8:
                ...
                if sensor_data[can_id]["value"] is not None and sensor_data[can_id]["timestamp"] is not None:
                    ...
                    save_to_sqlite(can_id, sensor_data[can_id]["sensor_name"], sensor_data[can_id]["timestamp"], sensor_data[can_id]["value"])
